...
def dot(matrix_a: list[list[float]], matrix_b: list[list[float]]) -> list[list[float]]:
    ...
    if c_a != r_b:
        assert f"Dot product cannot be performed on matrices with size: ({r_a},{c_a}), ({r_b},{c_b})"
...
def add_vector(matrix: list[list[float]], vector: list[float]) -> list[list[float]]:
    ...
    if c_m != v_c:
        assert f"Addition cannot be performed on a matrix and vector with size: ({r_m},{c_m}), (1, {v_c})"
